...
export class GlobalSecurityManager {
  private static instance: GlobalSecurityManager;
  private securityService: SecurityService;
  private readonly globalPolicyPath: string;
  private readonly globalStandardsPath: string;
  ...
  public static getInstance(): GlobalSecurityManager {
   ...
   */
  async enforceGlobalSecurity(serverPath: string, projectRoot?: string): Promise<{
    success: boolean;
   ...
   * 评估安全扫描结果是否符合全局策略
   */
  private evaluateSecurityResult(securityScan: SecurityScanResult, policy: GlobalSecurityPolicy): {
    success: boolean;
    message: string;
  } {
   ...
   * 获取全局安全策略
   */
  async getGlobalPolicy(): Promise<GlobalSecurityPolicy> {
    try {
      ...
      logger.error("读取全局安全策略失败", { error: (error as Error).message });
    }
   ...
   * 更新全局安全策略
   */
  async updateGlobalPolicy(updates: Partial<GlobalSecurityPolicy>): Promise<void> {
    const currentPolicy = await this.getGlobalPolicy();
    const newPolicy: GlobalSecurityPolicy = {
      ...currentPolicy,
      ...updates,
   ...
   * 获取全局安全标准
   */
  async getGlobalStandards(): Promise<GlobalSecurityStandards> {
    try {
      ...
      logger.error("读取全局安全标准失败", { error: (error as Error).message });
    }
   ...
   * 启用全局安全策略强制执行
   */
  async enableGlobalSecurity(strictMode: boolean = false): Promise<void> {
   ...
   * 禁用全局安全策略强制执行（仅记录模式）
   */
  async disableGlobalSecurity(): Promise<void> {
    await this.updateGlobalPolicy({
      enforced: false
    });
   ...
   * 确保默认标准文件存在
   */
  private async ensureDefaultStandards(): Promise<void> {
    if (!(await fs.pathExists(this.globalStandardsPath))) {
   ...
   * 获取默认安全策略
   */
  private getDefaultPolicy(): GlobalSecurityPolicy {
    return {
      enforced: true,
      minimumScore: 85,
      strictMode: false,
   ...
   * 获取默认安全标准
   */
  private getDefaultStandards(): GlobalSecurityStandards {
    return {
      codeRules: {
