 ...
 * 设置提示词处理器
 */
export function setupPromptHandlers(server) {
    // 列出可用提示词
 ...
 * 生成部署指导提示词
 */
function generateDeployGuide(serverType, serverPath) {
    const basePrompt = `
# MCP服务器部署指导
 ...
 * 生成故障排除指导提示词
 */
function generateTroubleshootGuide(errorMessage, serverName) {
 ...
 * 生成配置验证帮助提示词
 */
function generateValidationHelp(validationErrors) {
 ...
 * 生成服务器分析提示词
 */
function generateServerAnalysis(configContent) {
 ...
 * 生成备份策略提示词
 */
function generateBackupStrategy(serversCount) {
    const backupPrompt = `
# MCP配置备份策略建议
