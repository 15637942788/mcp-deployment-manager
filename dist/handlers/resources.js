 ...
 * 处理配置相关资源
 */
async function handleConfigResource(uri, configService) {
    switch (uri) {
 ...
 * 处理日志相关资源
 */
async function handleLogResource(uri) {
 ...
 * 处理系统相关资源
 */
async function handleSystemResource(uri, configService) {
    switch (uri) {
        case "system://status": {
            const configExists = await configService.configExists();
