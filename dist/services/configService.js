...
export class ConfigService {
    ...
    async ensureBackupDirectory() {
        try {
            ...
            logError(error, "创建备份目录失败", { path: this.backupDir });
    ...
    async readConfig() {
        try {
            ...
            logError(error, "读取配置文件失败", { path: this.cursorConfigPath });
    ...
    async writeConfig(config) {
        try {
            ...
            logError(error, "写入配置文件失败", { path: this.cursorConfigPath });
    ...
    async cleanupOldBackups() {
        try {
            ...
            configLogger.warn("清理旧备份文件失败", { error: error.message });
        }
    }
    /**
     * 列出所有备份
    ...
    async restoreFromBackup(backupFilename) {
        try {
            ...
            logError(error, "从备份恢复配置失败", { backupFilename });
     ...
     * 检查是否看起来像敏感数据
     */
    looksLikeSensitiveData(key, value) {
     ...
     * 添加或更新服务器配置（带强制安全检查）
     */
    async addServer(name, serverConfig, force = false) {
        try {
            // 读取当前配置
            ...
            logError(error, "添加服务器配置失败", { serverName: name });
            return {
                success: false,
                message: `部署服务器失败: ${error.message}`,
     ...
     * 确保原有配置安全且强制备份
     */
    async addServerWithProtection(name, serverConfig, serverPath, projectRoot, force = false, _skipSecurity = false // 已弃用：现在总是执行全局安全检查
    ) {
        try {
            ...
            logError(error, "受保护的服务器部署失败", { serverName: name, serverPath });
            return {
                success: false,
                message: `受保护的服务器部署失败: ${error.message}`,
                errors: [error.message]
            };
        }
    }
     ...
     */
    async addServerProtected(name, serverConfig, force, isOverwrite) {
        try {
            // 读取当前配置
            const currentConfig = await this.readConfig();
            // 记录操作前的状态
            ...
            logError(error, "受保护的服务器添加失败", { serverName: name });
            return {
                success: false,
                message: `受保护的服务器添加失败: ${error.message}`,
                errors: [error.message]
            };
        }
    }
     ...
     * 强制要求所有部署都经过安全扫描
     */
    async addServerWithSecurity(name, serverConfig, serverPath, projectRoot, force = false, skipSecurity = false) {
        try {
            ...
            logError(error, "安全部署失败", { serverName: name, serverPath });
            return {
                success: false,
                message: `安全部署失败: ${error.message}`,
                errors: [error.message]
            };
        }
    }
    ...
    async removeServer(name) {
        try {
            ...
            logError(error, "移除服务器配置失败", { serverName: name });
            return {
                success: false,
                message: `移除服务器失败: ${error.message}`,
                errors: [error.message]
            };
        }
    }
     ...
     * 创建配置备份（带注释）
     */
    async createBackupWithResult(_comment) {
        try {
     ...
     * 从备份恢复配置（返回结果）
     */
    async restoreFromBackupWithResult(backupFile) {
        try {
    ...
    async scanDirectory(directory) {
        try {
            ...
            logError(error, "扫描目录失败", { directory });
            return { servers: [] };
        }
    }
}
